...
export default function DiscoveryScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      console.log("getMeApi diary:", JSON.stringify(meData, null, 2));
      console.log("getFeedPostsApi:", JSON.stringify(feedData, null, 2));

      setCurrentUser(meData);
      setPosts(Array.isArray(feedData) ? feedData : []);
    } catch (error: any) {
      console.log("load diary error:", error);
      console.log("load diary status:", error?.response?.status);
      console.log("load diary response:", error?.response?.data);
